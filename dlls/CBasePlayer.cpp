	...
	if ( pev->takedamage )
	{
		...
		case HITGROUP_HEAD:
			if (bitsDamageType & (DMG_BULLET | DMG_CLUB)) {
				...
				if (flDamage > gSkillData.sk_player_head*100) {
					flDamage = gSkillData.sk_player_head*100; // damage cap
				}
				...
				if (flDamage > gSkillData.sk_player_chest*100) {
					flDamage = gSkillData.sk_player_chest*100; // damage cap
				}
			}
			break;
		case HITGROUP_CHEST:
		...
		case HITGROUP_STOMACH:
			...
			if (flDamage > gSkillData.sk_player_stomach*100) {
				flDamage = gSkillData.sk_player_stomach*100; // damage cap
			}
			break;
		case HITGROUP_LEFTARM:
		case HITGROUP_RIGHTARM:
			flDamage *= gSkillData.sk_player_arm;
			if (flDamage > gSkillData.sk_player_arm*100) {
				flDamage = gSkillData.sk_player_arm*100; // damage cap
			}
			break;
		case HITGROUP_LEFTLEG:
		case HITGROUP_RIGHTLEG:
			flDamage *= gSkillData.sk_player_leg;
			if (flDamage > gSkillData.sk_player_leg*100) {
				flDamage = gSkillData.sk_player_leg*100; // damage cap
			}
			break;
		default:
			break;
