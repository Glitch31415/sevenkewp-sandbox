...
while (flDamage > 1 && loops < 25)
	{
		...
		if (pEntity == NULL)
			break;
		UTIL_BeamPoints(vecSrc + (gpGlobals->v_up * -8) + (gpGlobals->v_right * 8), tr.vecEndPos, MODEL_INDEX("sprites/laserbeam.spr"), 0, 0, 1, 16, 0, RGBA(255, 255, 255, flDamage), 0, NULL, NULL, NULL);
		if ( fFirstBeam )
		{
