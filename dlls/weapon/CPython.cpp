...
void CPython::PrimaryAttack()
{
	...
	m_pPlayer->pev->punchangle = Vector(-10, g_engfuncs.pfnRandomLong ( -2, 2 ), 0);
int loops = 0;
while (flDamage > 1 && loops < 25)
	{
		...
		if ( fFirstBeam )
		{
			...
			fFirstBeam = 0;
			UTIL_BeamPoints(vecSrc + (gpGlobals->v_up * -3.5) + (gpGlobals->v_forward * 23) + (gpGlobals->v_right * 5.5), tr.vecEndPos, MODEL_INDEX("sprites/laserbeam.spr"), 0, 0, 1, 8, 0, RGBA(255, 255, 255, flDamage), 0, NULL, NULL, NULL);
