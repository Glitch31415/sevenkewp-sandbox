...
while (flDamage > 1 && loops < 25)
	{
		...
		if ( fFirstBeam )
		{
			...
			fFirstBeam = 0;
			UTIL_BeamPoints(vecSrc + (gpGlobals->v_up * -2) + (gpGlobals->v_forward * 24) + (gpGlobals->v_right * 6), tr.vecEndPos, MODEL_INDEX("sprites/laserbeam.spr"), 0, 0, 1, 8, 0, RGBA(255, 255, 255, flDamage), 0, NULL, NULL, NULL);
